...
{
    public class Verb_LaunchProjectileCE_With_ChangeAble_Projectile : Verb_LaunchProjectileCE
    {
        #region Constants
        ...
        private new Vector2 newTargetLoc = new Vector2(0, 0);
        private new Vector2 sourceLoc = new Vector2(0, 0);
        ...
        protected new CompCharges compCharges = null;
        protected new MagazineUser compAmmo = null;
        protected new CompFireModes compFireModes = null;
        protected new CompChangeableProjectile compChangeable = null;
        protected new CompReloadable compReloadable = null;
        ...
        public new VerbPropertiesCE VerbPropsCE => verbProps as VerbPropertiesCE;
        public new ProjectilePropertiesCE projectilePropsCE => Projectile.projectile as ProjectilePropertiesCE;

        // Returns either the pawn aiming the weapon or in case of turret guns the turret operator or null if neither exists
        public new Pawn ShooterPawn => CasterPawn ?? NEW_CE_Utility.TryGetTurretOperator(caster);
        public new Thing Shooter => ShooterPawn ?? caster;

        protected new CompCharges CompCharges
        {
            get
            {
        ...
        protected new float ShotHeight => (new CollisionVertical(caster)).shotHeight;
        ...
        protected new float ShootingAccuracy => Mathf.Min(CasterShootingAccuracyValue(caster), 4.5f);
        protected new float AimingAccuracy => Mathf.Min(Shooter.GetStatValue(CE_StatDefOf.AimingAccuracy), 1.5f); //equivalent of ShooterPawn?.GetStatValue(CE_StatDefOf.AimingAccuracy) ?? caster.GetStatValue(CE_StatDefOf.AimingAccuracy)
        protected new float SightsEfficiency => EquipmentSource?.GetStatValue(CE_StatDefOf.SightsEfficiency) ?? 1f;
        protected new virtual float SwayAmplitude => Mathf.Max(0, (4.5f - ShootingAccuracy) * (EquipmentSource?.GetStatValue(StatDef.Named("SwayFactor")) ?? 1f));

        // Ammo variables
        protected new MagazineUser CompAmmo
        {
            get
            {
                if (compAmmo == null && EquipmentSource != null)
                {
                    compAmmo = EquipmentSource.TryGetComp<MagazineUser>();
                }
                return compAmmo;
            }
        }
        ...
        protected new CompChangeableProjectile CompChangeable
        {
            get
            {
        ...
        public new CompReloadable CompReloadable
        {
            get
            {
        ...
        public override bool Available()
        {
            ...
            if (Projectile == null && !CompAmmo.loadedmag.TryGetComp<Gazine>().Props.secondaryammo.ammoTypes.Any(hghgh => hghgh.ammo == CompAmmo.CurrentAmmo) || (IsAttacking && CompAmmo != null && !CompAmmo.CanBeFiredNow))
            {
                CompAmmo?.TryStartReload("ababababababa");
                return false;
            }
            return true;
        }
        ...
        public virtual float CasterShootingAccuracyValue(Thing caster) => // ShootingAccuracy was split into ShootingAccuracyPawn and ShootingAccuracyTurret
        ...
        /// </summary>
        protected new virtual void ShiftTarget(ShiftVecReport report, bool calculateMechanicalOnly = false, bool isInstant = false)
        {
        ...
        /// I should probably remove the comments, but idk - Caula
        public float Recoil;
        ...
        protected new void GetSwayVec(ref float rotation, ref float angle)
        {
        ...
        public new virtual ShiftVecReport ShiftVecReportFor(LocalTargetInfo target)
        {
        ...
        public new float AdjustShotHeight(Thing caster, LocalTargetInfo target, ref float shotHeight)
        {
            ...
            var shooterHeight = NEW_CE_Utility.GetBoundsFor(caster).max.y;
            var coverHeight = NEW_CE_Utility.GetBoundsFor(cover).max.y;
            var centerOfVisibleTarget = (NEW_CE_Utility.GetBoundsFor(target.Thing).max.y - coverHeight) / 2 + coverHeight;
        ...
        public new bool CanHitTarget(LocalTargetInfo targ, out string report)
        {
            return CanHitTargetFrom(caster.Position, targ, out report);
        }

        public new virtual bool CanHitTargetFrom(IntVec3 root, LocalTargetInfo targ, out string report)
        {
            report = "";
        ...
        /// </summary>
        public new virtual void VerbTickCE()
        {
        }
        ...
        public new bool TryFindCEShootLineFromTo(IntVec3 root, LocalTargetInfo targ, out ShootLine resultingLine)
        {
            ...
            if (verbProps.requireLineOfSight)
            {
                ...
                Predicate<IntVec3> CanShootThroughCell = (IntVec3 cell) =>
                {
                    ...
                    if (cover != null && cover != ShooterPawn && cover != caster && cover != targetThing && !cover.IsPlant() && !(cover is Pawn && cover.HostileTo(caster)))
                    {
                        ...
                        Bounds bounds = NEW_CE_Utility.GetBoundsFor(cover);
